...
const registration_1 = require("../shared/registration");
const registration_2 = require("../modules/inventory/registration");
const registration_3 = require("../modules/order/registration");
const ProductStockReducedHandler_1 = require("../modules/inventory/infrastructure/event-handlers/ProductStockReducedHandler");
const OrderCreatedHandler_1 = require("../modules/order/infrastructure/event-handlers/OrderCreatedHandler");
const CreateOrderUseCase_1 = require("../modules/order/application/use-cases/CreateOrderUseCase");
// ============================================================================
...
(0, registration_1.registerSharedDependencies)(container);
(0, registration_2.registerInventoryModule)(container);
(0, registration_3.registerOrderModule)(container);
// ============================================================================
...
container.registerFactory('CreateOrderUseCase', () => {
    const orderRepo = container.resolve('IOrderRepository');
    const inventoryChecker = container.resolve('IInventoryChecker');
    const reduceStockUseCase = container.resolve('ReduceStockUseCase');
    const eventBus = container.resolve('IEventBus');
    return new CreateOrderUseCase_1.CreateOrderUseCase(orderRepo, inventoryChecker, reduceStockUseCase, eventBus);
});
...
const productStockReducedHandler = new ProductStockReducedHandler_1.ProductStockReducedHandler(logger);
const orderCreatedHandler = new OrderCreatedHandler_1.OrderCreatedHandler(logger);
// Subscribe handlers ke event bus
// CATATAN: Event type harus sama dengan constructor name class event
eventBus.subscribe('ProductStockReduced', productStockReducedHandler);
...
async function runDemo() {
    ...
    try {
        ...
        console.log('-----------------------------------------------------------');
        const createProductResult = await container.resolve('CreateProductUseCase')
            .execute({
            id: 'prod-laptop-001',
            name: 'Gaming Laptop',
        ...
        const createOrderResult = await container.resolve('CreateOrderUseCase')
            .execute({
            id: 'order-001',
        ...
        const failedOrderResult = await container.resolve('CreateOrderUseCase')
            .execute({
            id: 'order-002',
